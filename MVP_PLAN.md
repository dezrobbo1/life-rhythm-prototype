...
Status (Gate 7D2): **Observed behaviour, descriptive statistics, and the complete explicit-preference user loop are implemented through this slice.** Gate 7A's factual event contract is recorded in `app/docs/gate7a-behaviour-event-contract.md`; Gate 7B's read-only statistics and read-health rules are recorded in `app/docs/gate7b-descriptive-statistics-contract.md`; Gate 7C's strict preference storage and precedence foundation is recorded in `app/docs/gate7c-explicit-preferences-contract.md`; Gate 7D1's live scheduling integration and repair-authority boundary is recorded in `app/docs/gate7d1-explicit-preference-live-integration-contract.md`; Gate 7D2's user-facing create/edit/delete/export/clear and grounded placement-reason boundary is recorded in `app/docs/gate7d2-explicit-preference-controls-contract.md`. Gate 7 as a whole remains in progress.

Gate 7D is complete through explicit preferences. The next bounded product slice is **Gate 7E — Explainable Personal Duration Learning v0**: use trusted completion-duration observations to improve duration estimates conservatively, with sample counts, provenance, bounded confidence and user correction/reset. Do not infer preferred time-of-day from raw counts without an opportunity denominator.
