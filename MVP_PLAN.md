...
Status (Gate 7D1): **Observed behaviour, descriptive statistics, explicit-preference persistence/precedence, and the live preference-integration foundation are implemented through this slice.** Gate 7A's factual event contract is recorded in `app/docs/gate7a-behaviour-event-contract.md`; Gate 7B's read-only statistics and read-health rules are recorded in `app/docs/gate7b-descriptive-statistics-contract.md`; Gate 7C's strict preference storage and precedence foundation is recorded in `app/docs/gate7c-explicit-preferences-contract.md`; Gate 7D1's live-read, per-candidate lifetime, precedence-ranking, stale-edit and plan-repair integrity boundary is recorded in `app/docs/gate7d1-explicit-preference-live-integration-contract.md`. Gate 7 as a whole remains in progress.

Gate 7D1 intentionally does not expose the production preference editor yet. The next bounded slice is **Gate 7D2 — Explicit Preference Editing + Reasons/Controls v0**: provide inspect/create/edit/delete/export/clear controls, connect each confirmed mutation through the Gate 7D1 coordinator, and expose short grounded placement reasons. After Gate 7D is complete, introduce one bounded explainable learning capability. Keep necessary usability work alongside functionality; no visual-redesign detour is required before this progression.
