...
Status (Gate 7D2): **Observed behaviour, descriptive statistics, explicit-preference persistence/precedence, live scheduler integration, and the explicit-preference user-control loop are implemented through this slice.** Gate 7A's factual event contract is recorded in `app/docs/gate7a-behaviour-event-contract.md`; Gate 7B's read-only statistics and read-health rules are recorded in `app/docs/gate7b-descriptive-statistics-contract.md`; Gate 7C's strict preference storage and precedence foundation is recorded in `app/docs/gate7c-explicit-preferences-contract.md`; Gate 7D1's live-read, per-candidate lifetime, precedence-ranking, stale-edit and plan-repair integrity boundary is recorded in `app/docs/gate7d1-explicit-preference-live-integration-contract.md`; Gate 7D2's inspect/create/edit/delete/export/clear controls, plan reconciliation, friendly target catalogue and grounded placement-reason boundary are recorded in `app/docs/gate7d2-explicit-preference-controls-contract.md`. Gate 7 as a whole remains in progress.

Gate 7D is complete through the explicit-preference workflow. The next bounded learning slice is **Gate 7E — Explainable Personal Duration Learning v0**: derive a conservative duration estimate/range from trusted completion-duration observations, keep sample count/provenance visible, preserve explicit user authority, and use it only where the evidence contract supports it. Do not infer time-of-day acceptance rates from Gate 7B counts without an opportunity denominator. No visual-redesign detour is required.
