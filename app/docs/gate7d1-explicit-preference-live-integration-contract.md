...
Every accepted repair records the exact preference targets it actually incorporated, independently of that repair's primary trigger. Undo restores preference-repair attention from those recorded targets, because the explicit preference itself remains current. If another preference becomes pending after that repair but before Undo, the newly pending targets and the restored targets are unioned rather than one set replacing the other; the Undo instant becomes the repair-attention timestamp because Undo reintroduces the older plan. This applies equally when a preference was consumed by a manual, calendar, Reduced Day, missed-start, overrun, or dedicated preference repair. A later ensure can therefore reconcile the restored prior plan again rather than presenting it as preference-consistent truth.
